...
def test_user (test_db: Session) -> Dict:
    ...
    return {
        ...
        "name": existing_user.name if existing_user else new_user.name,
    }
...
def cleanup_test_data (test_db: Session, test_user: Dict) -> None:
    ...
    try:
        ...
        stmt = select (PushSubscription).where (PushSubscription.subscribable_id == test_user["id"])
        subscriptions = test_db.exec (stmt).all ()
