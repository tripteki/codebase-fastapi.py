...
class TestUserProfile:
    ...
    @pytest.mark.asyncio
    async def test_users_me_show_get (self, client: AsyncClient, auth_token: str) -> None:
        """
    ...
    @pytest.mark.asyncio
    async def test_users_me_update_put (self, client: AsyncClient, auth_token: str, test_user: dict) -> None:
        """
        ...
        assert "Design" in data["profile"]["interests"]

    @pytest.mark.asyncio
    async def test_users_me_update_multipart_post (self, client: AsyncClient, auth_token: str, test_user: dict) -> None:
        """
        Test POST /api/v1/users/me endpoint with avatar upload
        """
        import io

        files = {
            "avatar": ("avatar.png", io.BytesIO (self.PNG_AVATAR), "image/png")
        }
        data = {
            "name": test_user["name"],
            "email": test_user["email"],
        ...
        assert response.status_code == 200
        result = response.json ()
        assert result["profile"]["full_name"] == "Avatar User"
        assert result["profile"]["avatar"] is not None
        assert result["profile"]["avatar"].startswith ("avatars/")
        assert "/storage/avatars/" in result["profile"]["avatar_url"]

    @pytest.mark.asyncio
    async def test_users_me_update_multipart_rejects_invalid_avatar (
        self,
        client: AsyncClient,
        auth_token: str,
        test_user: dict
        ...
        """
        Test POST /api/v1/users/me rejects invalid avatar file type
        """
        import io

        files = {
            "avatar": ("avatar.txt", io.BytesIO (b"not-an-image"), "text/plain")
        }
        data = {
            "name": test_user["name"],
            "email": test_user["email"],
    ...
    @pytest.mark.asyncio
    async def test_users_me_interests_get (self, client: AsyncClient, auth_token: str) -> None:
        """
        Test GET /api/v1/users/me/interests endpoint
        """
        response = await client.get (
    ...
    @pytest.mark.asyncio
    async def test_users_me_accesses_get (self, client: AsyncClient, auth_token: str) -> None:
        """
        Test GET /api/v1/users/me/accesses endpoint
        """
        response = await client.get (
            "/api/v1/users/me/accesses",
            headers={"Authorization": f"Bearer {auth_token}"}
        )
