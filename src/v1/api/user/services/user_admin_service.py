...
class UserAdminService:
    ...
    @staticmethod
    async def all (userId: str, orders: List[Dict[str, Any]] = None, filters: List[Dict[str, Any]] = None, page: Dict[str, Any] = None) -> OffsetPagination[UserTransformerDto]:
        """
        Args:
            userId (str)
    ...
    async def delete (userId: str, id: str) -> UserTransformerDto:
        ...
        await eventEmitter.emit ("user.admin.deactivated", event)
        return UserTransformerDto (
            id=user.id,
            name=user.name,
            email=user.email,
    ...
    async def restore (userId: str, id: str) -> UserTransformerDto:
        ...
        await eventEmitter.emit ("user.admin.activated", event)
        return UserTransformerDto (
            id=user.id,
            name=user.name,
            email=user.email,
