...
class UserAuthDto (BaseModel):
    ...
    remember: Optional[bool] = Field (default=None, json_schema_extra={"example": True})
...
class UserMeUpdateValidatorDto (BaseModel):
    """
    UserMeUpdateValidatorDto (BaseModel)
    """
    name: str = Field (..., min_length=2, max_length=255, json_schema_extra={"example": "user"})
    email: EmailStr = Field (..., json_schema_extra={"example": "user@mail.com"})
    full_name: Optional[str] = Field (None, max_length=255, json_schema_extra={"example": "User Full Name"})
    interests: Optional[list[str]] = Field (default_factory=list, json_schema_extra={"example": ["Design"]})
    password: Optional[str] = Field (None, min_length=8, max_length=16, json_schema_extra={"example": "12345678"})
    password_confirmation: Optional[str] = Field (None, min_length=8, max_length=16, json_schema_extra={"example": "12345678"})
    ...
    def validate_password_confirmation (self) -> Self:
        ...
        Args:
            self
        Returns:
            UserMeUpdateValidatorDto
        """
        if self.password is not None and self.password != (self.password_confirmation or ""):
            raise ValueError ("_v1_user.auth.password_mismatch")
        if self.password is not None and self.password_confirmation is None:
            raise ValueError ("_v1_user.auth.password_confirmation_required")
        return self
