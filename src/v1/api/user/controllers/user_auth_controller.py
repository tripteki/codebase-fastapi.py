...
from src.v1.api.user.dtos.user_transformer_dto import UserAuthTransformerDto, UserMeTransformerDto, UserTransformerDto
...
@userAuthRouter.post (
    ...
    dependencies=[rateLimit (times=3, seconds=60)],
...
@userAuthRouter.put (
    "/refresh",
    status_code=Status.OK,
    dependencies=[rateLimit (times=3, seconds=60), Depends (security)],
    response_model=UserAuthTransformerDto,
    responses=getStandardResponses (unauthorized=True, forbidden=True)
)
async def refresh (
...
@userAuthRouter.post (
    "/register",
    status_code=Status.CREATED,
    dependencies=[rateLimit (times=5, seconds=60)],
...
@userAuthPublicRouter.post (
    "/verify-email/{email}",
    status_code=Status.OK,
    dependencies=[rateLimit (times=10, seconds=60), Depends (urlGuard)],
    response_model=UserTransformerDto,
