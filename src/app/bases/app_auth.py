...
class AppAuth:
    ...
    @classmethod
    def resolveRemember (cls, remember: Optional[bool] = None) -> bool:
        """
        Args:
    ...
    @classmethod
    def refreshTokenExpireDays (cls, remember: bool = True) -> int:
        """
        Args:
            remember (bool)
        Returns:
        ...
        if remember:
            return config.jwt_refresh_token_expire_days

        return config.jwt_session_refresh_token_expire_days

    @classmethod
    def createRefreshToken (cls, data: Dict[str, object], remember: bool = True) -> str:
        """
        Args:
            data (Dict[str, object])
            remember (bool)
        ...
        config = cls.config ()
        toEncode = data.copy ()
        expire = datetime.utcnow () + timedelta (days=cls.refreshTokenExpireDays (remember))
        toEncode.update ({
            "exp": expire,
            "iat": datetime.utcnow (),
            "type": "refresh",
