...
def queueFailedCommand (count: bool, queue: Optional[str], retry: bool, clear: bool, job_id: Optional[str]):
    """
    Manage failed queue jobs

    Usage:
        # List all failed jobs
        poetry run python3 src/cli.py queue:failed

        # List failed jobs in specific queue
        poetry run python3 src/cli.py queue:failed --queue user-admin-queue

        # Count failed jobs
        poetry run python3 src/cli.py queue:failed --count

        # Retry all failed jobs
        poetry run python3 src/cli.py queue:failed --retry

        # Retry failed jobs in specific queue
        poetry run python3 src/cli.py queue:failed --queue user-admin-queue --retry

        # Retry specific job
        poetry run python3 src/cli.py queue:failed --job-id abc123 --retry

        # Clear all failed jobs
        poetry run python3 src/cli.py queue:failed --clear

        # Clear failed jobs in specific queue
        poetry run python3 src/cli.py queue:failed --queue user-admin-queue --clear
    """
    try:
...
def _list_failed_jobs (queue: Queue, queue_name: str, job_ids: list):
    """
...
def _retry_specific_job (redis_conn: Redis, queue_name: str, job_id: str):
    """
